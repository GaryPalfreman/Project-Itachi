...
Itachi plans up to three read-only actions per question and answers through an ordered model chain. Current actions are sourced web search and bounded arithmetic. The local Face offers an **Autonomous** route; Streamlit offers an **Autonomous research** checkbox. Web search needs the separate per-question toggle. See [[Hosted_Testing]] and [[Protocol_and_State_Rules]].
